...
#include "ros/ros.h"
#include "fv_standalone/BlobMove.h"
#include "fv_standalone/BlobMoves.h"
#include "fv_standalone/CollectData.h"
//-------------------------------------------------------------------------------------------
namespace cv
{
...
namespace loco_rabbits
{
  ...
  ros::ServiceServer collect_data_srv;
...
bool collectData(fv_standalone::CollectData::Request &req, fv_standalone::CollectData::Response &res,
	cv::VideoCapture & cap, TBlobTracker2 & tracker, int n_rotate90)
{
	cv::Mat frame;
	std::string filepath = tracker.Params().saveDir + "/" + req.file_name;

	cap >> frame;
	
	cv::imshow("save calib file", frame);
	cv::imwrite(filepath, frame);


	Rotate90N(frame,frame,n_rotate90);

	tracker.Step(frame, frame);     

	const std::vector<TPointMove2> &data(tracker.Data());
	res.camera_index = 0;
    	res.camera_name = "Pi Camera";
    	res.width = 320;
    	res.height = 240;
    	...
    	{
      		fv_standalone::BlobMove &m(res.data[i]);
      		m.Pox = itr->Po.x;
      		m.Poy = itr->Po.y;
      		m.So = itr->So;
      		m.DPx = itr->DP.x;
...
TBlobTracker2Params::TBlobTracker2Params()
{
  ...
  NonZeroMax= 1.5;//1.5; // Maximum ratio of nonzero pixels in ROI over original keypoint area.
  ...
  saveDir = "./calib/images";

  cameraMatrix = (cv::Mat_<double>(3,3) <<  1.0736275616423244e+02, 0, 1.5950000000000000e+02, 0,
	  				1.0621546176147196e+02, 1.1950000000000000e+02, 0, 0, 1);
  distCoeffs = (cv::Mat_<double>(1,4) << -1.5184816225692993e-01, 4.6421917291141918e-01, -5.4591474319865330e-01, 1.5762562660526966e-01);
}
//-------------------------------------------------------------------------------------------
...
//  cv::fisheye::estimateNewCameraMatrixForUndistortRectify(cameraMatrix, distCoeffs, cv::Size(320,240), cv::Matx33d::eye(), newCamMat, 1);
  cv::fisheye::initUndistortRectifyMap(params_.cameraMatrix, params_.distCoeffs, cv::Matx33d::eye(), params_.cameraMatrix, cv::Size(320,240), CV_16SC2, map1, map2);
}
//-------------------------------------------------------------------------------------------
...
{
  std::vector<TBlobTracker2Params> params;
  ReadFromYAML(params, "params.yaml");
...
int main(int argc, char**argv)
{
  ...
  ros::init(argc, argv, "fv_node");
  ros::NodeHandle n;

  blobPub = n.advertise<fv_standalone::BlobMoves>("fv_standalone/blob_moves", 1000);
  ...
  collect_data_srv = n.advertiseService<fv_standalone::CollectData::Request, fv_standalone::CollectData::Response>("fv_standalone/collect_data", boost::bind(&collectData, _1, _2, boost::ref(cap), 																boost::ref(tracker), n_rotate90));
...
//	std::chrono::steady_clock::time_point tstart = std::chrono::steady_clock::now();
    ...
    cv::remap(frame, frame, tracker.map1, tracker.map2, cv::INTER_LINEAR, cv::BORDER_CONSTANT);
...
//    cv::putText(frame, "("+std::to_string(pos[0])+","+std::to_string(pos[1])+")", cv::Point(10,30), cv::FONT_HERSHEY_SIMPLEX, 0.5, cv::Scalar(255,255,0), 1);
    ...
    const std::vector<TPointMove2> &data(tracker.Data());
    fv_standalone::BlobMoves bms;
    bms.camera_index = 0;
    bms.camera_name = "Pi Camera";
    bms.width = 320;
    bms.height = 240;
    bms.data.resize(data.size());
    int i = 0;
    for(std::vector<TPointMove2>::const_iterator itr(data.begin()), itr_end(data.end()); itr!=itr_end; ++itr,++i)
    {
      fv_standalone::BlobMove &m(bms.data[i]);
      m.Pox = itr->Po.x;
      m.Poy = itr->Po.y;
      m.So = itr->So;
      m.DPx = itr->DP.x;
    ...
    else if(c=='l')  tracker.LoadCalib("calib.yaml");
